...
module.exports = class LuxioDevice extends Homey.Device {
  ...
  async onDiscoveryAvailable(discoveryResult) {
    // Ensure firmware version 200 or higher
    if (discoveryResult.txt?.version) {
      const version = parseInt(discoveryResult.txt.version);
      if (version < 200) {
        throw new Error(`This Luxio device runs an unsupported firmware version (v${discoveryResult.txt.version}). Please update the firmware at https://flash.luxio.lighting.`);
      }
    }
